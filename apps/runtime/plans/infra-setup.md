...
rootUser: admin
rootPassword: REDACTED-LOCAL-PASSWORD
mode: standalone
persistence:
  size: 50Gi
resources:
...
aws_secret_access_key: REDACTED-LOCAL-PASSWORD
...
kubectl create secret generic gitlab-root-password -n gitlab \
  --from-literal=password='REDACTED-LOCAL-PASSWORD'
...
helm install rancher rancher-stable/rancher \
  ...
  --set replicas=1 \
  --set bootstrapPassword=REDACTED-LOCAL-PASSWORD \
    ...
    "template": {
      "spec": {
        "hostAliases": [
          {
            "ip": "192.168.0.201",
            "hostnames": ["gitlab.example.com", "registry.example.com"]
          }
        ]
      }
    }
...
curl -sk -X PUT "https://gitlab.example.com/api/v4/groups/5" \
  -H "PRIVATE-TOKEN: $GITLAB_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"visibility": "public"}'

curl -sk -X PUT "https://gitlab.example.com/api/v4/projects/2" \
  -H "PRIVATE-TOKEN: $GITLAB_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"visibility": "public"}'
...
metadata:
  name: zomme
spec:
  gitRepo: https://gitlab.example.com/zomme/charts.git
  gitBranch: main
...
> **Nota:** A lista de charts do Rancher é lida do projeto GitLab `zomme/charts`, não diretamente do diretório `charts/` deste repositório. Ao alterar o chart aqui, publique a cópia atualizada em `https://gitlab.example.com/zomme/charts.git` e force o refresh do `ClusterRepo`.

Fluxo usado para publicar uma nova versão do chart no catálogo local:

```bash
git clone https://gitlab.example.com/zomme/charts.git /tmp/zomme-charts
...
> **Nota:** O registry usa MinIO externo para storage e é acessível via `https://registry.example.com`.
...
kubectl run minio-mc --restart=Never -n minio \
  ...
  --command -- sh -c "mc alias set minio http://minio:9000 admin 'REDACTED-LOCAL-PASSWORD' && mc mb minio/gitlab-registry --ignore-existing"
...
s3:
  ...
  accesskey: admin
  secretkey: REDACTED-LOCAL-PASSWORD
  region: us-east-1
...
helm upgrade buntime ./charts \
  --namespace zomme \
  --reuse-values \
  --set image.repository=ghcr.io/zommehq/buntime \
...
Em 2026-05-01, a release local estava usando `ghcr.io/zommehq/buntime:runtime-performance-resilience` com o secret `gitlab-registry`.
...
docker login registry.example.com -u root -p 'REDACTED-LOCAL-PASSWORD'

# Build e push
docker build -t ghcr.io/zommehq/minha-app:latest .
docker push ghcr.io/zommehq/minha-app:latest

# Pull
docker pull ghcr.io/zommehq/minha-app:latest
```
...
curl -sk -X POST "https://gitlab.example.com/api/v4/user/runners" \
  -H "PRIVATE-TOKEN: $GITLAB_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    ...
    [[runners]]
      ...
      [runners.kubernetes]
        ...
        [[runners.kubernetes.host_aliases]]
          ip = "192.168.0.201"
          hostnames = ["gitlab.example.com", "kas.gitlab.example.com", "registry.example.com"]
...
dig gitlab.example.com @127.0.0.1
```
...
# Testar
resolvectl query gitlab.example.com
```
...
192.168.0.201 gitlab.example.com
...
```bash
scp user@192.168.0.201:~/.kube/config ~/.kube/k3s-home-config
...
|---------|-----|-------------|
| GitLab | https://gitlab.example.com | root / REDACTED-LOCAL-PASSWORD |
| MinIO Console | https://console.minio.home | admin / REDACTED-LOCAL-PASSWORD |
| MinIO API | https://minio.home | admin / REDACTED-LOCAL-PASSWORD |
| Keycloak | https://keycloak.home | admin / REDACTED-LOCAL-PASSWORD |
| Rancher | https://rancher.home | admin / REDACTED-LOCAL-PASSWORD |
...
| GitLab Registry | https://registry.example.com | (usa credenciais GitLab) |
| GitLab KAS | https://kas.gitlab.example.com | (agentes Kubernetes) |
...
```bash
sshpass -p 'PASSWORD' ssh-copy-id -i ~/.ssh/id_rsa.pub user@192.168.0.201
```
...
**Problema:** Jobs falham porque pods não conseguem resolver `gitlab.example.com` ou `registry.example.com`.
**Erro:** `dial tcp: lookup gitlab.example.com: no such host`
**Solução:** Adicionar hostAliases no ConfigMap do runner:
```toml
[[runners.kubernetes.host_aliases]]
  ip = "192.168.0.201"
  hostnames = ["gitlab.example.com", "kas.example.com", "registry.home"]
```
...
services:
  - name: docker:24-dind
    alias: docker
    command: ["--tls=false", "--insecure-registry=registry.example.com"]
```
...
**Problema:** Rancher não consegue clonar repo Git do GitLab porque não resolve `gitlab.example.com`.
**Erro:** `fatal: unable to access 'https://gitlab.example.com/...': Could not resolve host: gitlab.example.com`
...
**Causa:** O Rancher indexa o `ClusterRepo` a partir do GitLab local (`https://gitlab.example.com/zomme/charts.git`). Mudanças feitas apenas no diretório `charts/` do repo principal não aparecem no catálogo até serem copiadas para esse repositório e o `ClusterRepo` ser reindexado.
**Solução:**
```bash
git clone https://gitlab.example.com/zomme/charts.git /tmp/zomme-charts
rsync -a --delete ./charts/ /tmp/zomme-charts/charts/buntime/
