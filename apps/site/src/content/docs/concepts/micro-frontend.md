...
description: How the CPanel shell hosts plugin UIs as isolated micro-frontends via <z-frame> and a typed MessageChannel.
sidebar:
  order: 4
---
...
For how plugins declare their UI, see [Plugin System](/concepts/plugin-system/). For
the worker pool that serves each iframe, see [Worker Pool](/concepts/worker-pool/).
...
[the Runtime](/concepts/runtime/) for details on the mechanism.
...
- [The Runtime](/concepts/runtime/) — `<base href>` injection, `X-Base`/`X-Not-Found` headers.
- [Plugin System](/concepts/plugin-system/) — manifest with `entrypoint`, `menus`, `injectBase`.
- [Worker Pool](/concepts/worker-pool/) — wrapper that serves the iframe HTML.
- [SPA App Shell guide](/guides/spa-app-shell/) — building a plugin SPA against the shell.
