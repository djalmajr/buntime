---
title: Runtime
description: The @buntime/runtime main process — Bun + Hono, startup flow, request handling, and multi-layer routing.
sidebar:
  order: 1
  label: Runtime
---
...
support. The main process orchestrates requests but **never executes
application code** — that work is isolated in workers (see
[Worker Pool](/concepts/worker-pool/)).
...
starts with `basePath` (validated upstream).
...
Full details in the [Runtime API Reference](/reference/api/).
...
:::note
Multi-value variables **always** use `:` (PATH style), never commas. This
applies to `RUNTIME_WORKER_DIRS` and `RUNTIME_PLUGIN_DIRS`.
:::

The full table — including core-plugin variables — lives in
[Operations → Environment variables](/ops/environment/).
...
- [Worker Pool](/concepts/worker-pool/) — LRU, lifecycle, sliding TTL, ephemeral concurrency.
- [Plugin System](/concepts/plugin-system/) — hooks, persistent vs serverless modes, manifest.
- [Micro-Frontend Architecture](/concepts/micro-frontend/) — z-frame, MessageChannel, isolation.
- [Runtime API Reference](/reference/api/) — endpoints, authentication, curl examples.
