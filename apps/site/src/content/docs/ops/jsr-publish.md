---
title: Publishing to JSR
description: How @buntime/shared is published to JSR via the GitHub Actions OIDC workflow — never from the CLI — and how to keep versions in sync.
sidebar:
  order: 10
---
...
For details on the package contents (exports, helpers, errors, logger), see [`@buntime/shared`](/packages/shared/). For the general project release flow (chart, runtime, CLI), see [Release flow](/ops/release-flow/).

## Why JSR instead of npm

The other monorepo packages (`@buntime/keyval`, `@buntime/plugin-*`) are private and consumed via `workspace:*` only within the mono. **External** plugins that run on the runtime need the types and utilities from `@buntime/shared` (`PluginImpl`, `PluginContext`, logger, errors, string/size/duration helpers). JSR was chosen for:
...
:::caution[Hard rule]
`jsr.json:version` and `package.json:version` must be identical.
:::
...
- Contents of `@buntime/shared` (logger, types, errors, utils): [`@buntime/shared`](/packages/shared/)
- The general project release flow: [Release flow](/ops/release-flow/)
