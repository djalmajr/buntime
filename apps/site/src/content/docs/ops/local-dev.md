...
description: Run Buntime on your desktop — monorepo watch mode, root .env, external plugins with hot reload, Docker Compose profiles, and three binary execution modes.
sidebar:
  order: 2
---
...
For environment variables and directories, see [Environments](/ops/environment/). For Kubernetes deploy, see [Helm charts](/ops/helm-kubernetes/).
...
:::note
The runtime does **not** automatically reload when `dist/plugin.js` changes. You must restart `bun dev` for the runtime to pick up the new build.
:::
