---
title: Logging
description: The Buntime runtime logging pipeline — global singleton logger, per-context child loggers, console and file transports, log levels, and request ID correlation.
sidebar:
  order: 4
---
...
This page covers the **runtime logger** (in-memory, written to stdout/file). For the plugin that captures and exposes these logs via SSE/HTTP at `/logs`, see [logs plugin](/plugins/logs/). For structured errors consumed by the logger, see [shared package](/packages/shared/).
...
- In production: `json` (machine-readable, easy to parse by the logs plugin or Loki)
...
Validation details and errors related to request ID in [Security](/ops/security/).
...
- **Logs persisted via plugin** (ring buffer + SSE at `/logs`): [logs plugin](/plugins/logs/)
- **Structured errors** (`ValidationError`, `NotFoundError`, etc.): [shared package](/packages/shared/)
- **Sensitive env var filtering in workers**: [Security](/ops/security/)
- **Prometheus metrics**: [metrics plugin](/plugins/metrics/)
