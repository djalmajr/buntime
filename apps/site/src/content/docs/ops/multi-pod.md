---
title: Multi-pod deployment
description: Scaling Buntime beyond one pod with the in-cluster turso-server, per-pod PVCs, and S3 backups — fully self-hosted.
sidebar:
  order: 7
---
...
:::note
Looking for the implementation details? See [turso-server](/ops/turso-server/)
for the wrapper itself (Go binary, admin REST, GC, backup endpoint). This
page is the operator-facing recipe.
:::
...
- **Backup CronJob** uses the hot-backup admin endpoint (`VACUUM INTO`
  ...
  [turso-server — Why Litestream does NOT work here](/ops/turso-server/#why-litestream-does-not-work-here)).
...
[turso-server — Restoring](/ops/turso-server/#restoring) for the
...
- [turso-server](/ops/turso-server/) — wrapper internals, REST,
  backup/restore details.
- [Storage](/concepts/storage/) — control vs data plane databases.
- [Helm & Kubernetes](/ops/helm-kubernetes/) — values reference.
- [plugin-turso](/plugins/turso/) — multi-tenant mode.
- [API reference](/reference/api/) — `RUNTIME_AUTH_DB_*` env vars.
