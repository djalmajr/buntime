...
description: Measure, gate, and tune Buntime performance with the local perf harness, scenarios, runtime tuning vars, gates, and CI checklists.
sidebar:
  order: 5
---

How to measure, gate, and tune Buntime performance. The local harness (`bun run perf` in `apps/runtime/`) covers the worker pool + routing; Rancher/k3s environments are in dated reports (referenced at the bottom). For the pool model itself, see [worker-pool](/concepts/worker-pool/).
...
- See also [storage](/concepts/storage/) about the in-memory caches.
...
- [worker-pool](/concepts/worker-pool/) — LRU model, sliding TTL, ephemeral queue.
- [storage](/concepts/storage/) — tunable in-memory caches, file stores.
