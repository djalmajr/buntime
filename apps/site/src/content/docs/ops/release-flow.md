---
title: Release flow
description: Dual versioning (chart vs runtime), when to run bump-version.ts, CI triggers, and the GitHub and self-hosted GitLab pipelines.
sidebar:
  order: 9
---
...
To regenerate charts before bumping, see [Helm & Kubernetes](/ops/helm-kubernetes/). To publish `@buntime/shared` to JSR, see [Publishing to JSR](/ops/jsr-publish/).

:::danger[Release rules]
Never run `bump-version.ts`, `git tag`, or `git push` without explicit user permission. Every new version **must** have its own release notes in `charts/release-notes.md` before publishing.
:::
