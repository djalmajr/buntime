---
title: turso-server
description: Multi-tenant supervisor around the official tursodb sync-server binary — dynamic namespaces, embedded-replica sync, hot backups.
sidebar:
  order: 8
---
...
the data plane on Turso DB. Lives at `apps/turso-server/`.
...
- [plugin-turso](/plugins/turso/) — plugin-turso multi-tenant mode.
- [API reference](/reference/api/) — ApiKeyStore sync mode.
- [Multi-pod deployment](/ops/multi-pod/) — end-to-end multi-pod guide.
