...
For plugin model details (lifecycle, `provides`, `getPlugin`, `manifest.yaml`), see [Plugin System](/concepts/plugin-system/).
...
- **Consumers:** plugins that explicitly need persistent KV storage should use `ctx.getPlugin<Kv>("@buntime/plugin-keyval")`. Gateway and proxy operational state should not use KeyVal as infrastructure; see [plugin-turso](/plugins/turso/) and [Storage](/concepts/storage/).
...
For durable SQL provider behavior, see [plugin-turso](/plugins/turso/). For the client package, see [@buntime/keyval](/packages/keyval/).
