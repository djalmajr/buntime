...
For the plugin model (lifecycle, `provides`, manifest), see [Plugin System](/concepts/plugin-system/). For a plugin focused on logs and diagnostics, see [Logs](/plugins/logs/).
...
:::note
**Plugin or request metrics are not in this plugin.** Other plugins expose their own metrics at separate endpoints — e.g. [plugin-keyval](/plugins/keyval/) at `/keyval/api/metrics/prometheus`, [plugin-gateway](/plugins/gateway/) at `/gateway/api/metrics`. Configure multiple `metrics_path` in Prometheus to scrape all of them.
:::
...
- [Plugin System](/concepts/plugin-system/) — plugin model, lifecycle, manifest.
- [Logs](/plugins/logs/) — companion plugin for SSE-based diagnostics.
- [Runtime Logging](/ops/logging/) — for correlating worker IDs in logs.
