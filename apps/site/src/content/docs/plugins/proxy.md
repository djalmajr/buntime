---
title: Proxy
description: Dynamic HTTP and WebSocket reverse proxy with static manifest rules and dynamic rules persisted through plugin-turso. Base path /redirects.
sidebar:
  order: 5
---

Dynamic HTTP and WebSocket reverse proxy, with static rules in the manifest and
dynamic rules persisted through [`@buntime/plugin-turso`](/plugins/turso/), so
proxy can run independently from KeyVal. Default mount base `/redirects`, enabled
by default.

## Overview

`@buntime/plugin-proxy` intercepts requests in the `onRequest` hook and attempts
...
Everything else (base path, dependencies, menu) follows the standard plugin
system schema — see [Plugin System](/concepts/plugin-system/).

### Storage direction

The plugin declares a dependency on [`@buntime/plugin-turso`](/plugins/turso/) and
...
- The plugin manifest depends on `@buntime/plugin-turso` for durable SQL access, not on [`@buntime/plugin-keyval`](/plugins/keyval/).
...
:::note
The `manifest.yaml` also documents `base` (path to strip before forwarding) and
`relativePaths` (convert absolute paths to relative in the response) as options
supported by the loader. The concept pages cover only the eight fields above —
use with care and validate via testing before production.
:::

### Pattern matching

JS regex against the pathname (not the full URL). Capture groups are positional
(`$1`, `$2`, …):
...
**Evaluation order:** static first, dynamic after. Within each group,
declaration order — *first match wins*. Place specific rules before catch-alls.
...
Missing variables keep the placeholder in the URL — this almost always results in
...
Path rewrite and `changeOrigin` operate on the upgrade the same way as in HTTP.
...
All routes are under `/{base}/admin/*` — default `/redirects/admin/*` (the admin
...
`200 OK` response with array of rules. Static rules appear with `readonly: true`,
...
Body is a `ProxyRule` without `id`. `201 Created` response returns the created
rule with a generated UUID `id` (plus `order`, `enabled`, `readonly: false`).
Body parameters follow the [Rule model](#rule-model-proxyrule) table.
...
Updates a dynamic rule — body with fields to change. Attempting to edit a static
...
Removes a dynamic rule. Static rules return `403` with
...
`publicRoutes` declares, per rule, paths that should bypass authentication. The
...
|--------------------------|----------|--------------------------------------------------------|
| [`@buntime/plugin-turso`](/plugins/turso/) | Yes | Durable SQL provider for `proxy_rules` |
...
Provision dynamic rules after the pod is ready — useful when the set of services
...
| Dynamic rules disappear after restart      | Confirm [`plugin-turso`](/plugins/turso/) is active and configured with durable local/sync storage |
...
See also [Security](/ops/security/).
