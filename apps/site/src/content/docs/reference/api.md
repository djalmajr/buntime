...
description: Internal REST API for health checks, worker/plugin management, admin sessions, and API key management.
sidebar:
  order: 1
---

Internal REST API for health checks, plugin/app management, admin/auth, and
API key management. Used by the CLI/TUI, the [CPanel](/platform/cpanel/), and CI
automation.

For the general architecture, see [the runtime concept](/concepts/runtime/). For the
pool that executes operations, see [Worker Pool](/concepts/worker-pool/).
...
Responses:
- `200 OK` — body `{ authenticated: true, principal: {...} }`, headers include `Set-Cookie: [REDACTED] HttpOnly; SameSite=Strict; Path=/; Max-Age=86400` (and `Secure` on HTTPS).
...
Response: `204` with `Set-Cookie: [REDACTED] Max-Age=0; Path=/; SameSite=Strict`.
...
:::note
See [Plugin System](/concepts/plugin-system/) for why the
...
- Auth: uses `/api/admin/session` exclusively. Does not depend on a separate
  authentication plugin.
- The CPanel cannot be blocked by an authentication plugin — its `manifest.yaml`
  marks `publicRoutes: { GET: ["/**"] }`, so the SPA bundle is always reachable;
...
- [The runtime concept](/concepts/runtime/) — `RUNTIME_API_PREFIX`, CSRF, headers.
- [Worker Pool](/concepts/worker-pool/) — `/api/workers/*` endpoints (metrics/stats).
- [Plugin System](/concepts/plugin-system/) — `POST /api/plugins/reload` and hot reload.
